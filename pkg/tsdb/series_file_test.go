...
func TestSeriesFile_Series(t *testing.T) {
	...
	for _, s := range series {
		if _, err := sfile.CreateSeriesListIfNotExists([][]byte{[]byte(s.Name)}, []models.Tags{s.Tags}, []uint64{}); err != nil {
			t.Fatal(err)
		}
	}
...
func TestSeriesFileCompactor(t *testing.T) {
	...
	if _, err := sfile.CreateSeriesListIfNotExists(names, tagsSlice, []uint64{}); err != nil {
		t.Fatal(err)
	}
...
func TestSeriesFile_DeleteSeriesID(t *testing.T) {
	...
	ids0, err := sfile.CreateSeriesListIfNotExists([][]byte{[]byte("m1")}, []models.Tags{nil}, []uint64{})
	if err != nil {
		t.Fatal(err)
	} else if _, err := sfile.CreateSeriesListIfNotExists([][]byte{[]byte("m2")}, []models.Tags{nil}, []uint64{}); err != nil {
		t.Fatal(err)
	...
	if err := sfile.DeleteSeriesID(ids0[0]); err != nil {
		t.Fatal(err)
	} else if _, err := sfile.CreateSeriesListIfNotExists([][]byte{[]byte("m1")}, []models.Tags{nil}, []uint64{}); err != nil {
		t.Fatal(err)
...
func TestSeriesFile_Compaction(t *testing.T) {
	...
	ids, err := sfile.CreateSeriesListIfNotExists(mms, tagSets, []uint64{})
	require.NoError(t, err)
...
func BenchmarkSeriesFile_Compaction(b *testing.B) {
	...
	if cachedCompactionSeriesFile == nil {
		...
		var ids []uint64
		for i := 0; i < n; i++ {
			tmp, err := sfile.CreateSeriesListIfNotExists([][]byte{[]byte("cpu")}, []models.Tags{models.NewTags(map[string]string{"region": fmt.Sprintf("r%d", i)})}, []uint64{})
			if err != nil {
				b.Fatal(err)
			}
			ids = append(ids, tmp...)

...
func TestFloatArrayEncodeAll(t *testing.T) {
	...
	for _, example := range examples {
		src := example
		var buf []byte
		buf, err := tsm.FloatArrayEncodeAll(src, buf)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}

		result, err := tsm.FloatArrayDecodeAll(buf, nil)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
...
func TestFloatArrayEncode_Compare(t *testing.T) {
	...
	var buf2 []byte
	buf2, err = tsm.FloatArrayEncodeAll(input, buf2)
	if err != nil {
		t.Fatalf("unexpected error: %v\nbuf: %db %x", err, len(buf2), buf2)
	}

	result, err := tsm.FloatArrayDecodeAll(buf2, nil)
	if err != nil {
		dumpBufs(buf1, buf2)
		t.Fatalf("unexpected error: %v\nbuf: %db %x", err, len(buf2), buf2)
	}
...
func TestFloatArrayEncodeAll_NaN(t *testing.T) {
	...
	for _, example := range examples {
		var buf []byte
		_, err := tsm.FloatArrayEncodeAll(example, buf)
		if err == nil {
...
func Test_FloatArrayEncodeAll_Quick(t *testing.T) {
	quick.Check(func(values []float64) bool {
		...
		var buf2 []byte
		buf2, err = tsm.FloatArrayEncodeAll(src, buf2)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
...
func TestDecodeFloatArrayAll_Empty(t *testing.T) {
	...
	var got []float64
	if _, err := tsm.FloatArrayDecodeAll(b, got); err != nil {
		t.Fatal(err)
	}
...
func TestFloatArrayDecodeAll_Simple(t *testing.T) {
	...
	buf := make([]float64, 8)
	got, err := tsm.FloatArrayDecodeAll(b, buf)
	if err != nil {
	...
	for _, n := range cases {
		b.Run(fmt.Sprintf("%d_seq", n), func(b *testing.B) {
			...
			b.Run("batch", func(b *testing.B) {
				...
				b.ResetTimer()
				for n := 0; n < b.N; n++ {
					if bufResult, err = tsm.FloatArrayEncodeAll(input, bufResult); err != nil {
						b.Fatal(err)
					} else {
						b.SetBytes(int64(len(bufResult)))
					}
	...
	for _, n := range cases {
		b.Run(fmt.Sprintf("%d_seq", n), func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				...
				got, err := tsm.FloatArrayDecodeAll(data, dst)
				if err != nil {
	...
	for _, size := range benchmarks {
		...
		b.Run(fmt.Sprintf("%d", size), func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				...
				got, err := tsm.FloatArrayDecodeAll(bytes, dst)
				if err != nil {
					b.Fatalf("unexpected error\n%s", err.Error())
				}
				if len(got) != size {

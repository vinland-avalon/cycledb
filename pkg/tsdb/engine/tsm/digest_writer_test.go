...
func TestEngine_DigestManifestNotWritten(t *testing.T) {
	f := MustTempFile(t, "")
	w, err := tsm.NewDigestWriter(f)
	if err != nil {
		t.Fatalf("NewDigestWriter: %v", err)
	}
	defer w.Close()

	ts := &tsm.DigestTimeSpan{}
	ts.Add(1, 2, 3, 4)

	if err := w.WriteTimeSpan("cpu", ts); err != tsm.ErrNoDigestManifest {
		t.Fatalf("exp: tsm.ErrNoDigestManifest, got: %v", err)
	}
}
...
func TestEngine_DigestReadSkipsManifest(t *testing.T) {
	...
	if err := w.WriteManifest(&tsm.DigestManifest{}); err != nil {
		t.Fatal(err)
	}
...
func TestEngine_DigestManifestDoubleWrite(t *testing.T) {
	...
	if err := w.WriteManifest(&tsm.DigestManifest{}); err != tsm.ErrDigestAlreadyWritten {
		t.Fatalf("exp: %s, got: %s", tsm.ErrDigestAlreadyWritten, err)
	}
}
...
func TestEngine_DigestManifestDoubleRead(t *testing.T) {
	...
	// Write the manifest.
	if err := w.WriteManifest(&tsm.DigestManifest{Dir: "test"}); err != nil {
		t.Fatal(err)
	}
	if err := w.Close(); err != nil {
		t.Fatal(err)
	...
	if _, err := r.ReadManifest(); err != tsm.ErrDigestManifestAlreadyRead {
...
func TestEngine_DigestWriterReader(t *testing.T) {
	...
	for {
		...
		exp := tsm.DigestTimeRange{Min: 1, Max: 2, N: 3, CRC: 4}

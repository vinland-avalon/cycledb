...
func TestBooleanArrayEncodeAll_NoValues(t *testing.T) {
	b, err := tsm.BooleanArrayEncodeAll(nil, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	var dec tsm.BooleanDecoder
	dec.SetBytes(b)
	if dec.Next() {
...
func TestBooleanArrayEncodeAll_Single(t *testing.T) {
	...
	b, err := tsm.BooleanArrayEncodeAll(src, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestBooleanArrayEncodeAll_Compare(t *testing.T) {
	...
	buf2, err = tsm.BooleanArrayEncodeAll(input, buf2)
	if err != nil {
		t.Fatalf("unexpected error: %v\nbuf: %db %x", err, len(buf2), buf2)
	}

	result, err := tsm.BooleanArrayDecodeAll(buf2, nil)
	if err != nil {
		dumpBufs(buf1, buf2)
		t.Fatalf("unexpected error: %v\nbuf: %db %x", err, len(buf2), buf2)
	}
...
func TestBooleanArrayEncodeAll_Quick(t *testing.T) {
	if err := quick.Check(func(values []bool) bool {
		...
		buf, err := tsm.BooleanArrayEncodeAll(src, nil)
		if err != nil {
			t.Fatal(err)
		}
...
func Test_BooleanArrayDecodeAll_Single(t *testing.T) {
	...
	got, _ := tsm.BooleanArrayDecodeAll(b, nil)
	if len(got) != 1 {
...
func Test_BooleanArrayDecodeAll_Multi_Compressed(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(fmt.Sprintf("%d_%0.2f", tc.n, tc.p), func(t *testing.T) {
			...
			enc := tsm.NewBooleanEncoder(tc.n)
			...
			got, err := tsm.BooleanArrayDecodeAll(b, nil)
			if err != nil {
...
func Test_BooleanBatchDecoder_Corrupt(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			dst, _ := tsm.BooleanArrayDecodeAll([]byte(c.d), nil)
			if len(dst) != 0 {
	...
	for _, n := range cases {
		...
		b.Run(fmt.Sprintf("%d_ran", n), func(b *testing.B) {
			...
			b.Run("batch", func(b *testing.B) {
				...
				b.ResetTimer()
				for n := 0; n < b.N; n++ {
					if bufResult, err = tsm.BooleanArrayEncodeAll(input, bufResult); err != nil {
						b.Fatal(err)
					}
				}
			})
...
func BenchmarkBooleanArrayDecodeAll(b *testing.B) {
	...
	for _, bm := range benchmarks {
		b.Run(fmt.Sprintf("%d", bm.n), func(b *testing.B) {
			...
			dst := make([]bool, size)
			for i := 0; i < b.N; i++ {
				res, _ := tsm.BooleanArrayDecodeAll(bytes, dst)
				if len(res) != size {

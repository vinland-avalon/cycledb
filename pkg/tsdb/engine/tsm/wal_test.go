...
func NewWAL(path string, maxConcurrentWrites int, maxWriteDelay time.Duration) *tsm.WAL {
	// EngineTags is only for metrics, not needed for tests
	return tsm.NewWAL(path, maxConcurrentWrites, maxWriteDelay, tsdb.EngineTags{})
}
...
func TestWALWriter_WriteMulti_Single(t *testing.T) {
	...
	p1 := tsm.NewValue(1, 1.1)
	p2 := tsm.NewValue(1, int64(1))
	p3 := tsm.NewValue(1, true)
	p4 := tsm.NewValue(1, "string")
	p5 := tsm.NewValue(1, ^uint64(0))

	values := map[string][]tsm.Value{
		"cpu,host=A#!~#float":    {p1},
		"cpu,host=A#!~#int":      {p2},
		"cpu,host=A#!~#bool":     {p3},
...
func TestWALWriter_WriteMulti_LargeBatch(t *testing.T) {
	...
	var points []tsm.Value
	for i := 0; i < 100000; i++ {
		points = append(points, tsm.NewValue(int64(i), int64(1)))
	}
...
func TestWALWriter_WriteMulti_Multiple(t *testing.T) {
	...
	p1 := tsm.NewValue(1, int64(1))
	p2 := tsm.NewValue(1, int64(2))

	exp := []struct {
		key    string
		values []tsm.Value
	}{
		{"cpu,host=A#!~#value", []tsm.Value{p1}},
		{"cpu,host=B#!~#value", []tsm.Value{p2}},
	}

	for _, v := range exp {
		_, err := w.WriteMulti(context.Background(), map[string][]tsm.Value{v.key: v.values})
		require.NoError(t, err)
	}
...
func TestWALWriter_WriteMultiDeleteRange_Multiple(t *testing.T) {
	...
	p1 := tsm.NewValue(1, 1.0)
	p2 := tsm.NewValue(2, 2.0)
	p3 := tsm.NewValue(3, 3.0)
	...
	de, ok := we.(*tsm.DeleteRangeWALEntry)
	require.True(t, ok)
...
func TestWAL_ClosedSegments(t *testing.T) {
	...
	_, err = w.WriteMulti(context.Background(), map[string][]tsm.Value{
		"cpu,host=A#!~#value": {
			tsm.NewValue(1, 1.1),
		},
	})
...
// GOARCH=386 go test -run TestWALSegmentReader_Corrupt -v ./tsdb/engine/tsm/
...
func TestWALRollSegment(t *testing.T) {
	...
	values := map[string][]tsm.Value{
		"cpu,host=A#!~#value": {tsm.NewValue(1, 1.0)},
		"cpu,host=B#!~#value": {tsm.NewValue(1, 1.0)},
		"cpu,host=C#!~#value": {tsm.NewValue(1, 1.0)},
	}
	_, err := w.WriteMulti(context.Background(), values)
	require.NoError(t, err)
...
func TestWAL_DiskSize(t *testing.T) {
	test := func(w *tsm.WAL, oldZero, curZero bool) {
...
func TestWriteWALSegment_UnmarshalBinary_WriteWALCorrupt(t *testing.T) {
	...
	for i := 0; i < len(b); i++ {
		...
		require.True(t, err == nil || err == tsm.ErrWALCorrupt)
	}
}
...
func TestDeleteWALEntry_UnmarshalBinary(t *testing.T) {
	...
	for _, example := range examples {
		w := &tsm.DeleteWALEntry{Keys: slices.StringsToBytes(example.In...)}
		b, err := w.MarshalBinary()
		require.NoError(t, err)

		out := &tsm.DeleteWALEntry{}
...
func BenchmarkWAL_WriteMulti_Concurrency(b *testing.B) {
	...
	for _, bm := range benchmarks {
		b.Run(fmt.Sprintf("concurrency-%d", bm.concurrency), func(b *testing.B) {
			points := map[string][]tsm.Value{}
			for i := 0; i < 5000; i++ {
				k := "cpu,host=A#!~#value"
				points[k] = append(points[k], tsm.NewValue(int64(i), 1.1))
			}
...
func mustSegmentReader(t *testing.T, w *tsm.WAL) (*os.File, *tsm.WALSegmentReader) {
	files, err := filepath.Glob(filepath.Join(w.Path(),
		fmt.Sprintf("%s*.%s", tsm.WALFilePrefix, tsm.WALFileExtension)))
	require.NoError(t, err)
	require.Equal(t, 1, len(files))
	...
	require.NoError(t, err)
	return f, tsm.NewWALSegmentReader(f)
}
...
func mustMarshalEntry(entry tsm.WALEntry) (tsm.WalEntryType, []byte) {

// Package tsm provides a TSDB in the Time Structured Merge tree format.
package tsm // import "cycledb/pkg/tsdb/engine/tsm"
...
func init() {
	tsdb.RegisterEngine("tsm", NewEngine)
}
...
var (
	tsmGroup                   = metrics.MustRegisterGroup("tsm")
...
func (e *Engine) Digest() (io.ReadCloser, int64, error) {
	...
	log, logEnd := logger.NewOperation(context.TODO(), e.logger, "Engine digest", "tsm_digest")
	defer logEnd()

	log.Info("Starting digest", zap.String("tsm_path", e.path))
	...
	log.Info("Digest written", zap.String("tsm_digest_path", digestPath), zap.Int64("size", fi.Size()))
...
// PrometheusCollectors returns all prometheus metrics for the tsm package.
...
func (e *Engine) WithLogger(log *zap.Logger) {
	e.logger = log.With(zap.String("engine", "tsm"))
...
func (e *Engine) WriteSnapshot() (err error) {
	...
	started := time.Now()
	log, logEnd := logger.NewOperation(context.TODO(), e.logger, "Cache snapshot", "tsm_cache_snapshot")
	defer func() {
...
func (s *compactionStrategy) compactGroup() {
	group := s.group
	log, logEnd := logger.NewOperation(context.TODO(), s.logger, "TSM compaction", "tsm_compact_group")
	defer logEnd()

	log.Info("Beginning compaction", zap.Int("tsm_files_n", len(group)))
	for i, f := range group {
		log.Info("Compacting file", zap.Int("tsm_index", i), zap.String("tsm_file", f))
	}
	...
	for i, f := range files {
		log.Info("Compacted file", zap.Int("tsm_index", i), zap.String("tsm_file", f))
	}
	log.Info("Finished compacting files",
		zap.Int("tsm_files_n", len(files)))
}
...
func (e *Engine) levelCompactionStrategy(group CompactionGroup, fast bool, level int) *compactionStrategy {
	...
	return &compactionStrategy{
		group:     group,
		logger:    e.logger.With(zap.Int("tsm_level", level), zap.String("tsm_strategy", "level")),
...
func (e *Engine) fullCompactionStrategy(group CompactionGroup, optimize bool) *compactionStrategy {
	s := &compactionStrategy{
		group:     group,
		logger:    e.logger.With(zap.String("tsm_strategy", "full"), zap.Bool("tsm_optimize", optimize)),
		fileStore: e.FileStore,
		compactor: e.Compactor,
		fast:      optimize,
		engine:    e,

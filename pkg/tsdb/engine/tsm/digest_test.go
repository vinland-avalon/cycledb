...
func TestDigest_None(t *testing.T) {
	...
	files := []string{}
	if err := tsm.Digest(dir, files, df); err != nil {
...
func TestDigest_One(t *testing.T) {
	...
	writes := map[string][]tsm.Value{
		"cpu,host=A#!~#value": []tsm.Value{a1},
	}
	MustWriteTSM(t, dir, 1, writes)

	files, err := filepath.Glob(filepath.Join(dir, fmt.Sprintf("*.%s", tsm.TSMFileExtension)))
	if err != nil {
		t.Fatal(err)
	}
...
func TestDigest_TimeFilter(t *testing.T) {
	...
	a2 := tsm.NewValue(2, 2.1)
	writes = map[string][]tsm.Value{
		"cpu,host=A#!~#value": []tsm.Value{a2},
	}
	MustWriteTSM(t, dir, 2, writes)

	a3 := tsm.NewValue(3, 3.1)
	writes = map[string][]tsm.Value{
		"cpu,host=A#!~#value": []tsm.Value{a3},
	}
	...
	if err := tsm.DigestWithOptions(dir, files, tsm.DigestOptions{MinTime: 2, MaxTime: 2}, df); err != nil {
		t.Fatalf("digest error: %v", err)
	}
...
func TestDigest_KeyFilter(t *testing.T) {
	...
	a2 := tsm.NewValue(2, 2.1)
	writes = map[string][]tsm.Value{
		"cpu,host=B#!~#value": []tsm.Value{a2},
	}
	MustWriteTSM(t, dir, 2, writes)

	a3 := tsm.NewValue(3, 3.1)
	writes = map[string][]tsm.Value{
		"cpu,host=C#!~#value": []tsm.Value{a3},
	}
	MustWriteTSM(t, dir, 3, writes)
	...
	if err := tsm.DigestWithOptions(dir, files, tsm.DigestOptions{
...
func TestDigest_Manifest(t *testing.T) {
	...
	digestFile := filepath.Join(dir, tsm.DigestFilename)
	...
	// Generate a manifest.
	mfest, err := tsm.NewDigestManifest(dir, files)
	if err != nil {
		t.Fatal(err)
	}
	...
	readManifest := func(name string) *tsm.DigestManifest {
		t.Helper()
	...
	fresh, reason := tsm.DigestFresh(dir, files, digest.ModTime())
	if !fresh {
		t.Fatalf("digest is stale: reason=%s", reason)
	}

	// Test that digest is stale if shard time is newer than digest time.
	fresh, _ = tsm.DigestFresh(dir, files, digest.ModTime().Add(1))
	if fresh {
	...
	fresh, _ = tsm.DigestFresh(dir, allfiles, digest.ModTime())
	if fresh {
		t.Fatalf("digest is fresh")
	}
	...
	fresh, _ = tsm.DigestFresh(dir, files, digest.ModTime())
	if fresh {
		t.Fatalf("digest is fresh")
	}

...
func TestEngine_DeleteWALLoadMetadata(t *testing.T) {
	for _, index := range tsdb.RegisteredIndexes() {
		t.Run(index, func(t *testing.T) {
			...
			if exp, got := 0, len(e.Cache.Values(tsm.SeriesFieldKeyBytes("cpu,host=A", "value"))); exp != got {
				t.Fatalf("unexpected number of values: got: %d. exp: %d", got, exp)
			}

			if exp, got := 1, len(e.Cache.Values(tsm.SeriesFieldKeyBytes("cpu,host=B", "value"))); exp != got {
				t.Fatalf("unexpected number of values: got: %d. exp: %d", got, exp)
			}
		})
	}
...
func TestEngine_Digest(t *testing.T) {
	...
	if err := e.Open(context.Background()); err != nil {
		t.Fatalf("failed to open tsm engine: %s", err.Error())
	}
	...
	exp := []span{
		span{
			...
			tspan: &tsm.DigestTimeSpan{
				Ranges: []tsm.DigestTimeRange{
					tsm.DigestTimeRange{
						Min: 1000000000,
						Max: 1000000000,
						N:   1,
...
func TestEngine_Backup(t *testing.T) {
	...
	e := tsm.NewEngine(1, idx, f.Name(), walPath, sfile.SeriesFile, opt).(*tsm.Engine)
	...
	for _, f := range files {
		if strings.HasSuffix(f, tsm.TmpTSMFileExtension) {
...
func TestEngine_SnapshotsDisabled(t *testing.T) {
	...
	e := tsm.NewEngine(1, idx, dir, walPath, sfile.SeriesFile, opt).(*tsm.Engine)
...
func BenchmarkBlockTypeToInfluxQLDataType(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		for j := 0; j < len(t); j++ {
			blockType = tsm.BlockTypeToInfluxQLDataType(t[j])
		}
	}
}
...
func TestEngine_WritePoints_TypeConflict(t *testing.T) {
	...
	for _, index := range tsdb.RegisteredIndexes() {
		t.Run(index, func(t *testing.T) {
			...
			got, err := e.Type([]byte(tsm.SeriesFieldKey("cpu,host=A", "value")))
			if err != nil {
			...
			values := e.Cache.Values([]byte(tsm.SeriesFieldKey("cpu,host=A", "value")))
...
// Engine is a test wrapper for tsm.Engine.
type Engine struct {
	*tsm.Engine
	root      string
	indexPath string
...
func NewEngine(tb testing.TB, index string) (*Engine, error) {
	...
	tsmEngine := tsm.NewEngine(1, idx, filepath.Join(root, "data"), filepath.Join(root, "wal"), sfile, opt).(*tsm.Engine)
...
func (e *Engine) Reopen() error {
	...
	e.Engine = tsm.NewEngine(1, e.index, filepath.Join(e.root, "data"), filepath.Join(e.root, "wal"), e.sfile, opt).(*tsm.Engine)
...
func (m *mockPlanner) Plan(lastWrite time.Time) ([]tsm.CompactionGroup, int64) { return nil, 0 }
func (m *mockPlanner) PlanLevel(level int) ([]tsm.CompactionGroup, int64)      { return nil, 0 }
func (m *mockPlanner) PlanOptimize() ([]tsm.CompactionGroup, int64)            { return nil, 0 }
func (m *mockPlanner) Release(groups []tsm.CompactionGroup)                    {}
func (m *mockPlanner) FullyCompacted() (bool, string)                          { return false, "not compacted" }
func (m *mockPlanner) ForceFull()                                              {}
func (m *mockPlanner) SetFileStore(fs *tsm.FileStore)                          {}

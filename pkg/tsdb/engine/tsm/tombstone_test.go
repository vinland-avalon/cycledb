...
func TestTombstoner_Add(t *testing.T) {
	...
	ts := tsm.NewTombstoner(f.Name(), nil)
...
func TestTombstoner_ReadV1(t *testing.T) {
	...
	if err := os.Rename(f.Name(), f.Name()+"."+tsm.TombstoneFileExtension); err != nil {
...
func mustReadAll(t *tsm.Tombstoner) []tsm.Tombstone {
	var tombstones []tsm.Tombstone
	if err := t.Walk(func(t tsm.Tombstone) error {
		b := make([]byte, len(t.Key))
		copy(b, t.Key)
		tombstones = append(tombstones, tsm.Tombstone{
			Min: t.Min,
			Max: t.Max,
			Key: b,
		})

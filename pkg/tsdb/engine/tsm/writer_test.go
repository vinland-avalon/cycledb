...
func TestTSMWriter_Write_Empty(t *testing.T) {
	var b bytes.Buffer
	w, err := tsm.NewTSMWriter(&b)
	if err != nil {
		t.Fatalf("unexpected error created writer: %v", err)
	}

	if err := w.WriteIndex(); err != tsm.ErrNoValues {
...
func TestTSMWriter_Write_NoValues(t *testing.T) {
	...
	if err := w.Write([]byte("foo"), []tsm.Value{}); err != nil {
...
func TestTSMWriter_Write_Single(t *testing.T) {
	...
	values := []tsm.Value{tsm.NewValue(0, 1.0)}
	...
	if got := binary.BigEndian.Uint32(b[0:4]); got != tsm.MagicNumber {
		t.Fatalf("magic number mismatch: got %v, exp %v", got, tsm.MagicNumber)
	}
...
func TestTSMWriter_Write_Multiple(t *testing.T) {
	...
	var data = []struct {
		...
		{"cpu", []tsm.Value{tsm.NewValue(0, 1.0)}},
		{"mem", []tsm.Value{tsm.NewValue(1, 2.0)}},
	}
...
func TestTSMWriter_WriteBlock_MaxKey(t *testing.T) {
	...
	if err := w.WriteBlock([]byte(key), 0, 0, nil); err != tsm.ErrMaxKeyLengthExceeded {
...
func TestTSMWriter_Write_MaxKey(t *testing.T) {
	...
	if err := w.Write([]byte(key), []tsm.Value{tsm.NewValue(0, 1.0)}); err != tsm.ErrMaxKeyLengthExceeded {
		t.Fatalf("expected max key length error writing key: %v", err)
	}
}

...
func TestFileStore_Read(t *testing.T) {
	...
	// Setup 3 files
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{tsm.NewValue(0, 1.0)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(1, 2.0)}},
		keyValues{"mem", []tsm.Value{tsm.NewValue(0, 1.0)}},
	}
...
func TestFileStore_SeekToAsc_FromStart(t *testing.T) {
	...
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{tsm.NewValue(0, 1.0)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(1, 2.0)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, 3.0)}},
	}
...
func TestFileStore_SeekToAsc_Duplicate(t *testing.T) {
	...
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{tsm.NewValue(0, 1.0)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(0, 2.0)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, 3.0)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, 4.0)}},
	}
...
func TestFileStore_SeekToAsc_BeforeStart(t *testing.T) {
	...
	// Setup 3 files
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{tsm.NewValue(1, 1.0)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, 2.0)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(3, 3.0)}},
	}
...
func TestFileStore_SeekToAsc_BeforeStart_OverlapFloat(t *testing.T) {
	...
	// Setup 3 files
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{tsm.NewValue(0, 0.0), tsm.NewValue(1, 1.0)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, 2.0)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(3, 3.0)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(0, 4.0), tsm.NewValue(2, 7.0)}},
	}
...
func TestFileStore_SeekToAsc_BeforeStart_OverlapInteger(t *testing.T) {
	...
	// Setup 3 files
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{tsm.NewValue(0, int64(0)), tsm.NewValue(1, int64(1))}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, int64(2))}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(3, int64(3))}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(0, int64(4)), tsm.NewValue(2, int64(7))}},
	}
	...
	// Search for an entry that exists in the second file
	buf := make([]tsm.IntegerValue, 1000)
	c := fs.KeyCursor(context.Background(), []byte("cpu"), 0, true)
	t.Cleanup(c.Close)
...
func TestFileStore_SeekToAsc_BeforeStart_OverlapUnsigned(t *testing.T) {
	...
	// Setup 3 files
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{tsm.NewValue(0, uint64(0)), tsm.NewValue(1, uint64(1))}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, uint64(2))}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(3, uint64(3))}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(0, uint64(4)), tsm.NewValue(2, uint64(7))}},
	}
	...
	// Search for an entry that exists in the second file
	buf := make([]tsm.UnsignedValue, 1000)
	c := fs.KeyCursor(context.Background(), []byte("cpu"), 0, true)
	t.Cleanup(c.Close)
...
func TestFileStore_SeekToAsc_BeforeStart_OverlapBoolean(t *testing.T) {
	...
	// Setup 3 files
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{tsm.NewValue(0, true), tsm.NewValue(1, false)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, true)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(3, true)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(0, false), tsm.NewValue(2, true)}},
	}
	...
	// Search for an entry that exists in the second file
	buf := make([]tsm.BooleanValue, 1000)
	c := fs.KeyCursor(context.Background(), []byte("cpu"), 0, true)
	t.Cleanup(c.Close)
...
func TestFileStore_SeekToAsc_BeforeStart_OverlapString(t *testing.T) {
	...
	// Setup 3 files
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{tsm.NewValue(0, "zero"), tsm.NewValue(1, "one")}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, "two")}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(3, "three")}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(0, "four"), tsm.NewValue(2, "seven")}},
	}
...
func TestFileStore_SeekToAsc_OverlapMinFloat(t *testing.T) {
	...
	// Setup 3 files
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{tsm.NewValue(1, 1.0), tsm.NewValue(3, 3.0)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, 2.0), tsm.NewValue(4, 4.0)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(0, 0.0), tsm.NewValue(1, 1.1)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, 2.2)}},
	}
...
func TestFileStore_SeekToAsc_OverlapMinInteger(t *testing.T) {
	...
	// Setup 3 files
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{tsm.NewValue(1, int64(1)), tsm.NewValue(3, int64(3))}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, int64(2)), tsm.NewValue(4, int64(4))}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(0, int64(0)), tsm.NewValue(1, int64(10))}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, int64(5))}},
	}
...
func TestFileStore_SeekToAsc_OverlapMinUnsigned(t *testing.T) {
	...
	// Setup 3 files
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{tsm.NewValue(1, uint64(1)), tsm.NewValue(3, uint64(3))}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, uint64(2)), tsm.NewValue(4, uint64(4))}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(0, uint64(0)), tsm.NewValue(1, uint64(10))}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, uint64(5))}},
	}
...
func TestFileStore_SeekToAsc_OverlapMinBoolean(t *testing.T) {
	...
	// Setup 3 files
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{tsm.NewValue(1, true), tsm.NewValue(3, true)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, true), tsm.NewValue(4, true)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(0, true), tsm.NewValue(1, false)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, false)}},
	}
...
func TestFileStore_SeekToAsc_Middle(t *testing.T) {
	...
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{tsm.NewValue(1, 1.0),
			tsm.NewValue(2, 2.0),
			tsm.NewValue(3, 3.0)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(4, 4.0)}},
	}
...
func TestFileStore_SeekToDesc_Duplicate(t *testing.T) {
	...
	// Setup 3 files
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{tsm.NewValue(0, 4.0)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(0, 1.0)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, 2.0)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, 3.0)}},
	}
...
func TestFileStore_SeekToDesc_AfterEnd_OverlapFloat(t *testing.T) {
	...
	// Setup 4 files
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{tsm.NewValue(8, 0.0), tsm.NewValue(9, 1.0)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, 2.0)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(3, 3.0)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(3, 4.0), tsm.NewValue(7, 7.0)}},
	}
...
func TestFileStore_SeekToDesc_AfterEnd_OverlapInteger(t *testing.T) {
	...
	// Setup 3 files
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{tsm.NewValue(8, int64(0)), tsm.NewValue(9, int64(1))}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, int64(2))}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(3, int64(3))}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(3, int64(4)), tsm.NewValue(10, int64(7))}},
	}
...
func TestFileStore_SeekToDesc_AfterEnd_OverlapUnsigned(t *testing.T) {
	...
	// Setup 3 files
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{tsm.NewValue(8, uint64(0)), tsm.NewValue(9, uint64(1))}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, uint64(2))}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(3, uint64(3))}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(3, uint64(4)), tsm.NewValue(10, uint64(7))}},
	}
...
func TestFileStore_SeekToDesc_AfterEnd_OverlapBoolean(t *testing.T) {
	...
	// Setup 3 files
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{tsm.NewValue(8, true), tsm.NewValue(9, true)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, true)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(3, false)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(3, true), tsm.NewValue(7, false)}},
	}
...
func TestFileStore_SeekToDesc_AfterEnd_OverlapString(t *testing.T) {
	...
	// Setup 3 files
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{tsm.NewValue(8, "eight"), tsm.NewValue(9, "nine")}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(2, "two")}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(3, "three")}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(3, "four"), tsm.NewValue(7, "seven")}},
	}
...
func TestKeyCursor_TombstoneRange_PartialFirst(t *testing.T) {
	...
	expValues := []tsm.Value{tsm.NewValue(0, 0.0), tsm.NewValue(2, 2.0)}
...
func TestKeyCursor_TombstoneRange_PartialFloat(t *testing.T) {
	...
	expValues := []tsm.Value{data[0].values[0], data[0].values[2]}
...
func TestKeyCursor_TombstoneRange_PartialInteger(t *testing.T) {
	...
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{
			tsm.NewValue(0, int64(1)),
			tsm.NewValue(1, int64(2)),
			tsm.NewValue(2, int64(3))}},
	}
...
func TestKeyCursor_TombstoneRange_PartialUnsigned(t *testing.T) {
	...
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{
			tsm.NewValue(0, uint64(1)),
			tsm.NewValue(1, uint64(2)),
			tsm.NewValue(2, uint64(3))}},
	}
...
func TestFileStore_Replace(t *testing.T) {
	...
	replacement := fmt.Sprintf("%s.%s", files[2], tsm.TmpTSMFileExtension)
...
func TestFileStore_Open_Deleted(t *testing.T) {
	...
	// Create 3 TSM files...
	data := []keyValues{
		keyValues{"cpu,host=server2!~#!value", []tsm.Value{tsm.NewValue(0, 1.0)}},
		keyValues{"cpu,host=server1!~#!value", []tsm.Value{tsm.NewValue(1, 2.0)}},
		keyValues{"mem,host=server1!~#!value", []tsm.Value{tsm.NewValue(0, 1.0)}},
	}
...
func TestFileStore_Apply(t *testing.T) {
	...
	var n int64
	if err := fs.Apply(context.Background(), func(r tsm.TSMFile) error {
		atomic.AddInt64(&n, 1)
		return nil
	}); err != nil {
...
func TestFileStore_Stats(t *testing.T) {
	...
	newFile := MustWriteTSM(t, dir, 4, map[string][]tsm.Value{
		"mem": []tsm.Value{tsm.NewValue(0, 1.0)},
	})

	replacement := fmt.Sprintf("%s.%s.%s", files[2], tsm.TmpTSMFileExtension, tsm.TSMFileExtension) // Assumes new files have a .tmp extension
	...
	for _, stat := range stats {
		if strings.HasSuffix(stat.Path, fmt.Sprintf("%s.%s.%s", tsm.TSMFileExtension, tsm.TmpTSMFileExtension, tsm.TSMFileExtension)) {
			found = true
		}
	}
	...
	newFile = MustWriteTSM(t, dir, 5, map[string][]tsm.Value{
		"mem": []tsm.Value{tsm.NewValue(0, 1.0)},
	})
...
func newTestFileStore(tb testing.TB, dir string) *tsm.FileStore {
	fs := tsm.NewFileStore(dir, tsdb.EngineTags{})
...
func TestFileStore_Observer(t *testing.T) {
	...
	// Setup 3 files
	data := []keyValues{
		keyValues{"cpu", []tsm.Value{tsm.NewValue(0, 1.0), tsm.NewValue(1, 2.0)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(10, 2.0)}},
		keyValues{"cpu", []tsm.Value{tsm.NewValue(20, 3.0)}},
	}
...
func newFileDir(tb testing.TB, dir string, values ...keyValues) ([]string, error) {
	...
	for _, v := range values {
		...
		newName := filepath.Join(filepath.Dir(f.Name()), tsm.DefaultFormatFileName(id, 1)+".tsm")
...
func MustTempFile(tb testing.TB, dir string) *os.File {
	f, err := os.CreateTemp(dir, "tsmtest")
	if err != nil {
...
func BenchmarkFileStore_Stats(b *testing.B) {
	...
	for i := 0; i < 1000; i++ {
		data = append(data, keyValues{"cpu", []tsm.Value{tsm.NewValue(0, 1.0)}})
	}

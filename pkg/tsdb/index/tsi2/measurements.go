...
type Measurement struct {
	...
	// todo(vinland): downgrade to measurement level, and flush to index file in measurement block
...
func (m *Measurement) SeriesIDSet() *tsdb.SeriesIDSet {
	...
	idsSet.ForEach(func(id uint64) {
		resSet.Add(m.IndexIdToSeriesFileId[SeriesIdWithMeasurementId(m.measurementId, id)])
	})
	return resSet
}
...
func (m *Measurement) SetIdMap(indexId, seriesFileId uint64) {
	m.IndexIdToSeriesFileId[indexId] = seriesFileId
}
